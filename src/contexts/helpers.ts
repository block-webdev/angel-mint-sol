...
import { Data, updateMetadata, Creator, createMetadata, createMasterEdition } from './metadata';
...
export const buyNFT = async (nftData: ContentRecord, nftKey: PublicKey, wallet: WalletContextState) => {
  ...
  let { mintAccount, tokenAccount } = await mintNewNFT(nftData, wallet);
...
export const mintNewNFT = async (nftData: ContentRecord, wallet: WalletContextState) => {
  ...
  transaction.add(
    SystemProgram.createAccount({
      fromPubkey: wallet.publicKey ? wallet.publicKey : ADMIN_KEY_PAIR.publicKey,
  ...
  transaction.add(
    Token.createInitMintInstruction(
      ...
      0,
      wallet.publicKey ? wallet.publicKey : ADMIN_KEY_PAIR.publicKey,
      wallet.publicKey ? wallet.publicKey : ADMIN_KEY_PAIR.publicKey,
    ),
  );
  transaction.add(
  ...
  await createMetadata(
    data,
    wallet.publicKey ? wallet.publicKey.toBase58() : ADMIN_KEY_PAIR.publicKey.toBase58(),
  ...
  // let result = await sendAndConfirmTransaction(solConnection, transaction, [wallet ? wallet : ADMIN_KEY_PAIR, mintAccount, tokenAccount]);
